...
namespace STOCK
{
    public partial class frmXuatxi : Form
    {
        tb_SYS_USER _user;
        int right;
        CONGTY _cty;
        ...
        public frmXuatxi(tb_SYS_USER user, int right)
        {
            InitializeComponent();
        ...
        {
            int space1 = 90;
            int width1 = lb_ngay.Width + 3 + dtp.Width + space1 + lb_sophieu.Width + 3 + tb_sophieu.Width + space1 + lb_chietkhau.Width + 3 + tb_ck.Width + space1 +  lb_trangthai.Width + 3 + cbb_trangthai.Width;
            ...
            lb_chietkhau.Top = lb_ngay.Top;
            lb_chietkhau.Left = tb_sophieu.Right + space1;

            tb_ck.Top = lb_chietkhau.Top - 3;
            tb_ck.Left = lb_chietkhau.Right + 3;

            lb_trangthai.Top = lb_ngay.Top;
            lb_trangthai.Left = tb_ck.Right + space1;

            cbb_trangthai.Top = lb_trangthai.Top - 3;
            cbb_trangthai.Left = lb_trangthai.Right + 3;

            int space2 = 30;
            int width2 = lb_donvi2.Width + 3 + cbb_donvixuat.Width + space2 + lb_nhacungcap.Width + 3 + cbb_khachhang.Width;
            ...
            cbb_khachhang.Top = lb_nhacungcap.Top - 3;
            cbb_khachhang.Left = lb_nhacungcap.Right + 3;
        ...
        private void Chungtu_Info(tb_CHUNGTU ct)
        {
            ...
            tb_DONVI dvi = _dvi.getItem(cbb_donvixuat.SelectedValue.ToString());
            _tbs = _seq.getItem("XSI@" + DateTime.Today.Year.ToString() + "@" + dvi.KYHIEU);
            if (_tbs == null)
            {
                _tbs = new tb_SYS_SEQUENCE();
                _tbs.SEQNAME = "XSI@" + DateTime.Today.Year.ToString() + "@" + dvi.KYHIEU;
                _tbs.SEQVALUE = 1;
                _seq.add(_tbs);
            }
            if (_them)
            {
                ...
                ct.SCT = _tbs.SEQVALUE.Value.ToString("000000") + @"/" + DateTime.Today.Year.ToString().Substring(2, 2) + @"/XSI/" + dvi.KYHIEU;
            ...
            ct.MADVI = cbb_donvixuat.SelectedValue.ToString();
            ct.MADVI2 = cbb_khachhang.SelectedValue.ToString();
            ct.GHICHU = tb_ghichu.Text;
            ct.TRANGTHAI = Convert.ToInt32(cbb_trangthai.SelectedValue.ToString());
            ct.SOLUONG = _SOLUONG;
            if (tb_ck.Text != "")
                ct.CHIETKHAU = Convert.ToInt32(tb_ck.Text);
            else ct.CHIETKHAU = null;
        ...
        private void Chungtu_CT_Info(tb_CHUNGTU ct)
        {
            ...
            for (int i = 0; i < dataGridView2.RowCount; i++)
            {
                if (dataGridView2.Rows[i].Cells["TENHH"].Value != null)
                {
                    ...
                    if (tb_ck.Text != "")
                        ctct.CHIETKHAU = Convert.ToInt32(tb_ck.Text);
                    else ctct.CHIETKHAU = null;
        ...
        {
            cbb_khachhang.DataSource = _kh.getList();
            ...
            if (dataGridView2.Columns[e.ColumnIndex].Name == "BARCODE")
            {
                ...
                if (cv != null)
                {
                    ...
                    if (barcode.StartsWith("."))
                    {
                        ...
                        if (tb_ck.Text != "")
                            dataGridView2.Rows[e.RowIndex].Cells["THANHTIEN"].Value = Convert.ToDouble(dataGridView2.Rows[e.RowIndex].Cells["SOLUONGCT"].Value) * Convert.ToDouble(dataGridView2.Rows[e.RowIndex].Cells["DONGIA"].Value) * (1 - Convert.ToDouble(tb_ck.Text) / 100);
                        else dataGridView2.Rows[e.RowIndex].Cells["THANHTIEN"].Value = Convert.ToDouble(dataGridView2.Rows[e.RowIndex].Cells["SOLUONGCT"].Value) * Convert.ToDouble(dataGridView2.Rows[e.RowIndex].Cells["DONGIA"].Value);
                    }
                    else
                    {
        ...
        private void tb_ck_TextChanged(object sender, EventArgs e)
        {
            foreach (DataGridViewRow row in dataGridView2.Rows)
            {
                if (row.IsNewRow) continue;
                if (tb_ck.Text != "")
                    row.Cells["THANHTIEN"].Value = Convert.ToInt32(row.Cells["SOLUONGCT"].Value) * Convert.ToDouble(row.Cells["DONGIA"].Value) * (1 - Convert.ToDouble(tb_ck.Text) / 100);
                else
                {
                    row.Cells["THANHTIEN"].Value = Convert.ToInt32(row.Cells["SOLUONGCT"].Value) * Convert.ToDouble(row.Cells["DONGIA"].Value);
                }
            }
        }
    }
